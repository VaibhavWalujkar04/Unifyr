...
import { FaUser, FaGraduationCap, FaBriefcase, FaHeart, FaInfoCircle } from 'react-icons/fa';

const GetStartedForm = () => {
  const [currentPage, setCurrentPage] = useState(0);
  const [formData, setFormData] = useState({});

  // Animation for initial form rendering
  const fadeInScale = useSpring({
    ...
    const pages = [
      { title: "Personal Information", icon: FaUser, description: "Please provide your personal details.", fields: ["fullName", "dateOfBirth", "gender", "nationality", "address", "contactNumber", "email"] },
      { title: "Educational Background", icon: FaGraduationCap, description: "Tell us about your education.", fields: ["highestDegree", "fieldOfStudy", "institution", "graduationYear", "gpa"] },
      { title: "Professional Experience", icon: FaBriefcase, description: "Share your professional experience.", fields: ["totalExperience", "currentEmployer", "jobTitle", "employmentDuration", "keyResponsibilities"] },
      { title: "Areas of Interest", icon: FaHeart, description: "Let us know your areas of interest.", fields: ["areasOfInterest", "preferredJobPostings"] },
      { title: "Additional Information", icon: FaInfoCircle, description: "Any additional information?", fields: ["securityClearance", "references", "additionalNotes"] },
    ];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex items-center justify-center px-4 py-10">
        <animated.div style={fadeInScale} className="bg-white shadow-2xl rounded-lg p-8 max-w-2xl w-full border border-gray-200 relative">
          {/* Progress Indicator */}
  ...
  const AnimatedButton = ({ children, primary, onClick, disabled, type }) => {
    ...
    return (
      <animated.button
        ...
        onClick={onClick}
        disabled={disabled}
        type={type} // Use the type prop for button type (submit or button)
